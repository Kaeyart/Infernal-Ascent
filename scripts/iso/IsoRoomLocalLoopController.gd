...
func _wire_room_deferred() -> void:
	...
	last_status = "Run room ready. Defeat enemies, claim a boon, choose a gate."
...
func _advance_same_scene(choice_data: Dictionary) -> void:
	...
	last_status = "Choice selected: %s. Completed %d/%d." % [
		choice_name,
		rooms_completed,
		rooms_until_run_end
	]
...
func _reset_room_cycle_deferred() -> void:
	...
	_wire_patron_flow_signals()
	_advance_in_progress = false
	last_status = "Next room cycle started. Enemies respawned."
