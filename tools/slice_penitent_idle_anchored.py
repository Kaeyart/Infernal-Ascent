...
def make_contact_sheet(exports: list[dict]) -> None:
    ...
    out = OUT_DIR / "_contact_sheet_idle_anchored.png"
    sheet.save(out)
...
def main() -> None:
    ...
    for entry in entries:
        ...
        # Bottom-center anchor. This is what removes the sliding.
        ...
        print(f"saved {out_name} canvas={canvas_w}x{canvas_h}")

    manifest = {
        "source": str(source),
        "layout": "4 rows x 4 columns",
        "rows": ROWS,
        "method": "bottom-center anchored idle slicing",
        "exports": exports,
    }

    (OUT_DIR / "_manifest_idle_anchored.json").write_text(json.dumps(manifest, indent=2))
